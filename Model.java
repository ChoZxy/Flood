...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        ...
        if (place.col != _width - 1) {
            action.accept(pl(place.row, place.col + 1));
        }
        if (place.row != _height - 1) {
            action.accept(pl(place.row + 1, place.col));
        }
    }
    ...
    boolean[] colorsPresent() {
        boolean[] colorarr = new boolean[_ncolors];
        for (int u = 0; u < _ncolors; u++) {
            colorarr[u] = false;
        }

        for (int n = 0; n < _cells.length; n++) {
            for (int m = 0; m < _cells[n].length; m++) {
                colorarr[_cells[n][m]] = true;
            }
        }
        return colorarr;
    }
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        ...
        if (get(start) != color || !result.add(start)) {
            return result;
        }
    ...
    Set<Place> adjacentCells(int color) {
        ...
        Set<Place> result = new HashSet<>(_active);
        undo();
        result.removeAll(_active);
    ...
    /**@ return getter to access _active in other file. */

    public HashSet<Place> getActive() {
        return _active;
    }
